...
async def chat_with_ai(request: AnalysisRequest):
    ...
    try:
        ...
        # Process questions sequentially for better performance
        for idx, q in enumerate(questions):
            ...
            if (all(kw in question_lower for kw in ["well", "lat", "long"]) or (
                ...
                # Create a mock request for extraction
                mock_request = Request(scope={'type': 'http', 'headers': []})
                extraction_result = await extract_well_coordinates(mock_request)
                ...
                if request.filename:
                    ...
                    for filename in files:
                        ...
                        if df is not None:
                            ...
                            all_headers += f"\n**Headers in `{filename}`:** Error reading file.\n"
                    answers[idx] = f"Q{idx+1}: {q}\n{all_headers}"
                continue
                    ...
                    for filename in files:
                        ...
                        if df is not None:
                            ...
                            **Sample Data:**
                            ...
                            try:
                                response = llm.invoke(prompt)
                                all_summaries += f"**{filename}:** {str(response.content).strip()}\n\n"
                            except Exception as e:
                                all_summaries += f"**{filename}:** Error generating summary: {str(e)}\n\n"
                        else:
                            all_summaries += f"**{filename}:** Error reading file.\n\n"
                    answers[idx] = f"Q{idx+1}: {q}\n{all_summaries}"
                continue
            
            # Handle file system scanning requests
            if any(word in question_lower for word in ["list", "folders", "subfolders", "csv files", "directory", "scan", "find files"]):
                try:
                    # First check for extracted files from zip
                    extracted_files = []
                    for root, dirs, files in os.walk(UPLOAD_DIR):
                        for file in files:
                            if file.lower().endswith('.csv'):
                    ...
                    # Also scan the general file system
                    scan_result = scan_file_system(".")
                    if "error" in scan_result:
                        answers[idx] = f"Q{idx+1}: {q}\n❌ Error scanning file system: {scan_result['error']}"
                        continue
                    
                    response_text = f"Q{idx+1}: {q}\n\n📁 **File System Scan Results:**\n\n"
                    response_text += f"**Total Folders:** {scan_result['total_folders']}\n"
                    response_text += f"**Total CSV Files:** {scan_result['total_csv_files']}\n"
                    response_text += f"**Extracted CSV Files:** {len(extracted_files)}\n\n"
                    
                    if extracted_files:
                        response_text += "**📦 Extracted CSV Files (from ZIP):**\n"
                        for csv_file in extracted_files:
                            response_text += f"- **{csv_file['name']}**\n"
                            response_text += f"  - Path: `{csv_file['relative_path']}`\n"
                            response_text += f"  - Size: {csv_file['size']} bytes\n\n"
                    
                    if scan_result['csv_files']:
                        response_text += "**📄 CSV Files in File System:**\n"
                        for csv_file in scan_result['csv_files']:
                            response_text += f"- **{csv_file['name']}**\n"
                            response_text += f"  - Full Path: `{csv_file['full_path']}`\n"
                            response_text += f"  - Relative Path: `{csv_file['relative_path']}`\n"
                            response_text += f"  - Size: {csv_file['size']} bytes\n\n"
                    
                    if not extracted_files and not scan_result['csv_files']:
                        response_text += "No CSV files found in the current directory and subdirectories.\n\n"
                    
                    if scan_result['folders']:
                        response_text += "**📂 Folders Found:**\n"
                        for folder in scan_result['folders'][:20]:  # Limit to first 20 folders
                            response_text += f"- **{folder['name']}** (`{folder['relative_path']}`)\n"
                        
                        if len(scan_result['folders']) > 20:
                            response_text += f"\n... and {len(scan_result['folders']) - 20} more folders.\n"
                    
                    answers[idx] = response_text
                    continue
                except Exception as e:
                    answers[idx] = f"Q{idx+1}: {q}\n❌ Error scanning file system: {str(e)}"
                    continue
            ...
            if any(word in question_lower for word in ["cluster", "clustering", "kmeans", "group data", "find groups"]):
                ...
                result = clustering_analysis_tool(filename_param, columns, n_clusters)
                answers[idx] = f"Q{idx+1}: {q}\n{result}"
                continue
            
            # Default: use Gemini LLM for general questions
            if request.filename:
                df = get_cached_file_data(request.filename)
                if df is None:
                    answers[idx] = f"Q{idx+1}: {q}\n❌ File not found."
