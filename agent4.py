    ...
    try:
        ...
        for idx, q in enumerate(questions):
            ...
            if (all(kw in question_lower for kw in ["well", "lat", "long"]) or (
                ...
                # If prompt is exactly 'extract all well locations from my data' or very similar, just output table
                if question_lower.strip() in [
                    "extract all well locations from my data",
                    "extract all well locations from my data.",
                    "extract all well locations from my data!",
                    "extract all well locations from my data?"
                ]:
                    ...
                    answers[idx] = f"Q{idx+1}: {q}\n\n{table_md}"
                    continue
                # Otherwise, restore previous behavior: save to CSV and provide download link
                extracted_rows = []
                seen = set()
                for fname, df in iter_uploaded_dataframes():
                    if fname in seen:
                ...
                for fname, df in iter_uploaded_dataframes():
                    ...
                    try:
                        ...
                        if lat_col and long_col:
                            ...
                            subset = df[cols].copy()
                            subset = subset.dropna(subset=[str(lat_col), str(long_col)])
                            if not well_col:
                                subset.insert(0, "Well Name", pd.Series([f"Well_{i+1}" for i in range(len(subset))], index=subset.index))
                                subset.columns = ["Well Name", "Latitude", "Longitude"]
                            else:
                ...
                for fname, df in iter_uploaded_dataframes():
                    if fname.lower().startswith(filename_param.lower()):
                        ...
                        try:
                            ...
                            counts = {int(k) if isinstance(k, (int, float, str)) and str(k).isdigit() else k: int(v) for k, v in value_counts.items()}
...
async def extract_well_coordinates(request: Request):
    ...
    for filename in files:
        ...
        if lat_col and long_col:
            ...
            if not well_col:
                well_names = pd.Series([f"Well_{i+1}" for i in range(len(subset))], index=subset.index)
